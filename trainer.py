...
class Model:
    def __init__(self, opt):
        ...
        self.criterion = SupConLoss(temperature=opt.temp)
    ...
    def train(self):
        ...
        pretrained_weights = torch.load(
            '/vulcan/scratch/koutilya/projects/Domain_Adaptation/SFS_net_ws_exp/SFS_net_ws_exp8/SFS_net_da-99999.pth.tar')[
            'primary_net_state_dict']
        ...
        saved_models = glob.glob(
            os.path.join('/vulcan/scratch/koutilya/projects/Domain_Adaptation/SFS_net_ws_exp/SFS_net_ws_exp8',
                         'SFS_net_da-*.pth.tar'))
        saved_iters = [int(s.split('-')[1].split('.')[0]) for s in saved_models]
        if (iteration == None):
            recent_id = saved_iters.index(max(saved_iters))
            iteration = max(saved_iters)
        else:
            recent_id = saved_iters.index(iteration)
        ...
        autoenc.cuda()
        # net.load_state_dict(model_state['primary_net_state_dict'])
        pretrained_weights_spoof_model = torch.load('data/opts/train_without_depth_lr_2e-5_drop_5_with_rgb_res34_lighthing_rgb_divided_recon_image/exp1_2stagev2_stage2_seed2/fold1/checkpoints/model_190.pth')
        self.model.load_state_dict(pretrained_weights_spoof_model['model_state_dict'])
    ...
    def train_epoch(self):
        ...
        self.batch_idx = 0
        for batch_idx, (rgb_data,rgb_data1, depth_data, ir_data, target) in enumerate(self.train_loader):
            
            self.batch_idx = batch_idx
            rgb_data = rgb_data.to(self.device)
            rgb_data1 = rgb_data1.to(self.device)
            # depth_data = depth_data.to(self.device)
            # ir_data = ir_data.to(self.device)
            #
            target = target.to(self.device)
            real_recon_image = self.autoenc(rgb_data)
            # real_estimated_normal, real_estimated_albedo, real_estimated_light = self.net(real_recon_image)
            ir_data, depth_data, real_estimated_light = self.net(real_recon_image)
            # diff_lighting = torch.log(rgb_data/real_recon_image)
            diff_lighting = (rgb_data / real_recon_image)
            # images = torch.cat([rgb_data, rgb_data1], dim=0)


            self.optimizer.zero_grad()
            
            output = self.model(rgb_data, depth_data, ir_data,diff_lighting)
            # bsz = target.shape[0]
    ...
    def test_epoch(self):
        ...
        for batch_idx, (rgb_data,rgb_data1, depth_data, ir_data, target) in enumerate(self.test_loader):
            rgb_data = rgb_data.to(self.device)
            depth_data = depth_data.to(self.device)
            ir_data = ir_data.to(self.device)
            target = target.to(self.device)
    ...
    def load_checkpoint(self):      # Load current checkpoint if exists
        ...
        fin_path = '/vulcanscratch/shlokm/Chalearn_challenge/SupContrast/data/opts/train_without_depth_lr_2e-5_drop_5_with_rgb_res34_lighthing_rgb_divided_recon_image/exp1_2stagev2_stage2_seed2/fold1/checkpoints/model_100.pth'
