...
class AggBlock(nn.Module):
    def __init__(self, layer_lvl, layers, agg_type='concat', channel_attention=False):
        ...
        if self.agg_type == 'concat':
            ...
            if use_lighting:
                self.conv1 = nn.Conv2d(inplanes * 4, inplanes, kernel_size=1)
            elif use_albedo_ablation:
                self.conv1 = nn.Conv2d(inplanes * 2, inplanes, kernel_size=1)

    def forward(self, prev_x, rgb_x, depth_x, ir_x, lighting_x):
        if self.agg_type == 'concat':
            if use_lighting:
                x = torch.cat((rgb_x, depth_x, ir_x, lighting_x), dim=1)
                x = nn.functional.relu(self.conv1(x))
            elif use_albedo_ablation:
                # x = torch.cat((rgb_x, depth_x), dim=1)
                x = torch.cat((rgb_x, ir_x), dim=1)
                x = nn.functional.relu(self.conv1(x))
        ...
        return x
    def _load_pretrained_weights_moco(self, weights):
        pretrained_weights = self.state_dict()
        moco_weights = weights['state_dict']

        # print(pretrained_weights.keys())
        # print(weights.keys())
        # import pdb
        # pdb.set_trace()
        # module.encoder_q.rgb_backbone
        for k, v in moco_weights.items():
            k = k.replace('module.encoder_q.rgb_backbone.','')
            if 'num_batches_tracked' in k or 'ch_att' in k:
                continue
            # print(k)
            if k in pretrained_weights.keys():
                print('found '+str(k))
...
class ResNetDLAS(nn.Module):
    def __init__(self, block, layers, DLAS_type='A', pretrained=None):
        ...
        if self.DLAS_type == 'A':
            ...
            if use_depth:
                ...
                if use_lighting:
                    self.layer4 = _make_layer(block, 256 * 4, 512, layers[3], stride=2)
                if use_albedo_ablation:
                    self.layer4 = _make_layer(block, 256 * 2, 512, layers[3], stride=2)
            else:
            ...
            if use_lighting:
                self.agg_layer4 = AggBlock(layer_lvl=4, layers=layers[3], agg_type='concat')
        ...
        self.lighting_backbone = ResNetCaffe(layers[:3], channel_attention=self.channel_attention)
    ...
    def forward(self, x, y, z, w):
        ...
        if use_depth:
            ...
            if use_lighting:
                w_layer1, w_layer2, w_layer3 = self.lighting_backbone(w)

            if use_lighting:
                x = torch.cat((x_layer3, y_layer3, z_layer3, w_layer3), dim=1)
            if use_albedo_ablation:
                # x = torch.cat((x_layer3, y_layer3), dim=1)
                x = torch.cat((x_layer3, z_layer3), dim=1)
                # print(x.shape)
            else:
            ...
            if use_lighting:
                agg_layer1 = self.agg_layer1(None, x_layer1, y_layer1, z_layer1, w_layer1)
                agg_layer2 = self.agg_layer2(agg_layer1, x_layer2, y_layer2, z_layer2, w_layer2)
                agg_layer3 = self.agg_layer3(agg_layer2, x_layer3, y_layer3, z_layer3, w_layer3)
            elif use_albedo_ablation:
                # agg_layer1 = self.agg_layer1(None, x_layer1, y_layer1, None, None)
                # agg_layer2 = self.agg_layer2(agg_layer1, x_layer2, y_layer2, None, None)
                # agg_layer3 = self.agg_layer3(agg_layer2, x_layer3, y_layer3, None, None)
                agg_layer1 = self.agg_layer1(None, x_layer1, None, z_layer1, None)
                agg_layer2 = self.agg_layer2(agg_layer1, x_layer2, None, z_layer2, None)
                agg_layer3 = self.agg_layer3(agg_layer2, x_layer3, None, z_layer3, None)
            else:
                agg_layer1 = self.agg_layer1(None, x_layer1, y_layer1, z_layer1, None)
                agg_layer2 = self.agg_layer2(agg_layer1, x_layer2, y_layer2, z_layer2, None)
                agg_layer3 = self.agg_layer3(agg_layer2, x_layer3, y_layer3, z_layer3, None)

                # agg_layer4 = self.agg_layer4(agg_layer3, x_layer3, y_layer3, z_layer3)
    ...
    def _load_pretrained_weights(self, pretrained):
        if pretrained == 'mcs2018':
            weights = torch.load(
                '/vulcan/scratch/shlok/ChaLearn_liveness_challenge/models_pertrained_onbig_data/resnet_caffe_mcs_orgl.pth',
                map_location='cpu')
            weights_moco = torch.load(
                '/vulcan/scratch/shlok/ChaLearn_liveness_challenge/moco/all_images/checkpoint_0103.pth.tar',
                map_location='cpu')
            weights_moco = torch.load(
                '/vulcan/scratch/shlok/ChaLearn_liveness_challenge/moco/only_real_images/checkpoint_0067.pth.tar',
                map_location='cpu')
        elif pretrained == 'afad_lite_g_exp1':
            weights = torch.load(
                '/media2/a.parkin/codes/Liveness_challenge/models/pretrained/resnet_caffe_afad_lite_g_exp1.pth',
                map_location='cpu')
        elif pretrained == 'afad_lite_g_exp1_mcs2018':
            weights = torch.load(
                '/media2/a.parkin/codes/Liveness_challenge/models/pretrained/resnet_caffe_afad_lite_g_exp1_mcs.pth',
                map_location='cpu')
        elif pretrained == 'afad_lite_a_exp1_mcs2018':
            weights = torch.load(
                '/media2/a.parkin/codes/Liveness_challenge/models/pretrained/resnet_caffe_afad_lite_a_exp1_mcs.pth',
                map_location='cpu')
        elif pretrained == 'umdfaces_cce_epoch30':
            weights = torch.load(
                '/media2/a.parkin/codes/Liveness_challenge/models/pretrained/umdfaces_exp1_epoch30.pth',
                map_location='cpu')
        elif pretrained == 'umdfaces_arc_margin_epoch30':
            weights = torch.load(
                '/media2/a.parkin/codes/Liveness_challenge/models/pretrained/umdfaces_exp2_epoch30.pth',
                map_location='cpu')
        ...
        self.lighting_backbone._load_pretrained_weights(weights)
        ...
        for k, v in pretrained_weights.items():
            ...
            if use_depth:
                if (self.DLAS_type in ['A', 'B', 'C']) and (k == '0.weight'):
                    ...
                    if use_lighting and use_depth:
                        weight = torch.cat((weight, weight, weight, weight), dim=1)
                        print(weight.shape)
                    elif use_albedo_ablation:
...
def resnetDLAS_152(pretrained=False, **kwargs):
    """Constructs a ResNet-34 model.
    Args:
        pretrained (bool): If True, returns a model pre-trained on ImageNet
    """
    print('resnetDLAS_152')
    model = ResNetDLAS(BasicBlock, [3, 8, 36, 3], DLAS_type='A', pretrained=False)
    return model
