...
class ResNetDLAS(nn.Module):
    def __init__(self, block, layers,num_classes=128, DLAS_type='A', pretrained=None):
    ...
    def _load_pretrained_weights(self, pretrained):
        if pretrained == 'mcs2018':
            weights = torch.load('/vulcan/scratch/shlok/ChaLearn_liveness_challenge/models_pertrained_onbig_data/resnet_caffe_mcs_orgl.pth', map_location='cpu')
...
def resnetDLAS_A(pretrained=False,num_classes=128, **kwargs):
    """Constructs a ResNet-34 model.
    Args:
        pretrained (bool): If True, returns a model pre-trained on ImageNet
    """
    print(pretrained)
    model = ResNetDLAS(BasicBlock, [1, 2, 5, 3],num_classes=128, DLAS_type='A', pretrained=pretrained)
    return model
