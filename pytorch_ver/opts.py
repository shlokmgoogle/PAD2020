...
def get_opts():
    ...
    opt.fold = 1
    opt.data_root = '/vulcan/scratch/shlok/ChaLearn_liveness_challenge/CASIA-SURF/'
    opt.data_list = '/vulcan/scratch/shlok/ChaLearn_liveness_challenge/data/lists/folds_by_fakes/fold{fold_n}/'.format(fold_n=opt.fold)
    
    opt.out_root = '/vulcanscratch/shlokm/Chalearn_challenge/pytorch_ver/train_with_depth_lr_1e-5_drop_5__OULU_sfsnet_plus/'
    opt.out_path = os.path.join(opt.out_root,opt.exp_name,'fold{fold_n}'.format(fold_n=opt.fold))
    
    ### Dataloader options ###
    opt.nthreads = 32
    opt.batch_size = 2 #280
    opt.ngpu = 2
