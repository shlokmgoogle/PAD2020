...
from .architectures.resnet_caffe import *
# from .architectures.resnet_caffe import resnet_v2
# from .architectures import ResNext
# from .architectures.resnet_caffe import resnet_siam
...
def get_model(net_type, clf_type, pretrained=False, output_size=2):
    ...
    elif net_type == 'ResNet152DLAS_A':
        # model = resnext_dlas.resnext50DLAS(pretrained=pretrained)
        model = resnet_caffe_DLAS.resnetDLAS_152(pretrained)
    else:
    ...
    if clf_type == 'linear':
        if use_depth:
            ...
            # classifier = nn.Linear(100352, output_size)
            classifier = nn.Linear(12800, output_size)
